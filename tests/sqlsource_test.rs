...
use open_ontologies::sqlsource::{SqlDriver, detect_driver, duckdb_target};

#[test]
fn detects_postgres() {
    assert_eq!(
        detect_driver("postgres://u:p@h/db").unwrap(),
        SqlDriver::Postgres
    );
    assert_eq!(
        detect_driver("postgresql://u@h:5432/db").unwrap(),
        SqlDriver::Postgres
    );
}
...
fn detects_duckdb_variants() {
    assert_eq!(
        detect_driver("duckdb:///tmp/x.duckdb").unwrap(),
        SqlDriver::DuckDb
    );
    assert_eq!(
        detect_driver("duckdb:/tmp/x.duckdb").unwrap(),
    ...
    assert_eq!(
        detect_driver("/var/data/warehouse.duckdb").unwrap(),
        SqlDriver::DuckDb
    );
