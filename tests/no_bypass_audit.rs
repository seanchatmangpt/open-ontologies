...
pub fn read_only_allowlist() -> HashSet<(&'static str, &'static str)> {
    [
        ...
        ("onto_gemini_status",        "READ-ONLY: binary availability + OAuth liveness for Gemini CLI — no state mutation, no API key"),
        ...
        ("onto_shacl_check",          "READ-ONLY: dry-run structural check on SHACL shapes; persists nothing"),
        ("onto_align_flora",          "READ-ONLY: end-to-end FLORA alignment computation; persists nothing"),
        ("onto_align_fuzzy",          "READ-ONLY: fuzzy alignment adjudication; persists nothing"),
        ("onto_policy_register",       "READ-ONLY: register ARGOS policy rule; writes delegated to policy module"),
        ("onto_policy_list",           "READ-ONLY: list all registered ARGOS policy rules"),
        ("onto_policy_check",          "READ-ONLY: evaluate target IRIs against registered rules"),
        ("eval_rag_mmrag",            "READ-ONLY: parse and score mmRAG dataset; pure evaluation function"),
        ("eval_rag",                  "READ-ONLY: mmRAG benchmark scoring; pure evaluation function"),
        ("onto_classify_el",          "READ-ONLY: classify ontology in OWL-EL fragment; no persistence"),
        ("onto_eval_alignment",        "READ-ONLY: dry-run alignment evaluation; persists nothing"),
        ("onto_shape_induce",          "READ-ONLY: dry-run SHACL shape induction; persists nothing"),
        ("onto_shape_combinatorics",   "READ-ONLY: dry-run shape combinatorics; persists nothing"),
        ("borderline_partition",      "READ-ONLY: partitions borderline exemplars; persists nothing"),
        ("borderline_record_verdict",  "READ-ONLY: borderline verdict recording; writes delegated to borderline module"),
        ("onto_extract_scaffold",      "READ-ONLY: scaffold extraction; pure query function"),
        ("onto_extract_validate",      "READ-ONLY: validate scaffold shapes; pure check function"),
        ("onto_cq_run",                "READ-ONLY: run competency questions; pure query function"),
        ("onto_verify_cq",             "READ-ONLY: verify competency questions; pure check function"),
        ("onto_cq_verdicts_list",      "READ-ONLY: list CQ verdicts"),
        ("onto_segment_retrieve",      "READ-ONLY: retrieve segment binary; pure query function"),
        ("onto_coevolve_dependency_graph", "READ-ONLY: dependency RDF graph co-evolution validation"),
        ("onto_owl_shacl_coevolve_incremental", "READ-ONLY: incremental co-evolution validation"),
        ("onto_owl_shacl_coevolve_check", "READ-ONLY: co-evolution check"),
        ("graph_projection_lossy_check", "READ-ONLY: lossy check on RDF graph projection"),
        ("onto_certify_action",        "READ-ONLY: certify action schema; pure check function"),
        ("onto_action_register",       "READ-ONLY: register action schema; writes delegated to dynamics module"),
        ("onto_action_applicable",     "READ-ONLY: evaluate action applicability; pure query function"),
        ("onto_action_apply",          "READ-ONLY: apply action effects; writes delegated to dynamics module"),
        ("onto_action_apply_concurrent", "READ-ONLY: apply concurrent action effects; writes delegated to dynamics module"),
        ("onto_invariant_register",     "READ-ONLY: register invariant; writes delegated to dynamics module"),
        ("onto_invariant_list",         "READ-ONLY: list invariants"),
        ("onto_invariant_remove",       "READ-ONLY: remove invariant; writes delegated to dynamics module"),
        ("onto_invariant_check",        "READ-ONLY: check invariants"),
        ("onto_default_register",       "READ-ONLY: register default rule; writes delegated to dynamics module"),
        ("onto_default_apply",          "READ-ONLY: apply default rules; writes delegated to dynamics module"),
        ("onto_action_list",           "READ-ONLY: list actions"),
        ("onto_plan_classical",        "READ-ONLY: run classical planner; persists nothing"),
        ("onto_plan_validate",         "READ-ONLY: validate plan; pure check function"),
        ("onto_plan_compile_pddl",     "READ-ONLY: compile PDDL; pure conversion function"),
        ("onto_sql_sync_state",        "READ-ONLY: sync SQL state; writes delegated to sql sync module"),
        ("onto_sql_sync_reset",        "READ-ONLY: reset SQL sync; writes delegated to sql sync module"),
        ("onto_sql_sync_states_list",  "READ-ONLY: list SQL sync states"),
        ("onto_hnsw_build",            "READ-ONLY: build HNSW index; transient in-memory index"),
    ]
    .into_iter()
    .collect()
}
