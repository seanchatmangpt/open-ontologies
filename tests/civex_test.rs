...
fn base_frame() -> ActionFrame {
    ...
    ActionFrame {
        ...
        dependent_queries: vec![
            ...
            "SELECT (COUNT(?x) AS ?n) WHERE { ?x a <http://www.w3.org/2002/07/owl#Class> }"
                .to_string(),
...
fn benign_addition_to_well_formed_ontology_executes() {
    ...
    assert!(
        result
            .certificate
            .assumptions
            .contains(&"structural_only".to_string())
    );
}
...
fn locked_iri_target_rejects_regardless_of_utility() {
    ...
    assert_eq!(
        result.certificate.verdict,
        Verdict::Reject,
        "locked IRI should hard-reject; got rationale: {}",
        result.certificate.rationale
    );
...
fn structural_mode_records_structural_only_assumption() {
    ...
    assert!(
        has_structural,
        "structural mode should record structural_only assumption; got: {:?}",
    ...
    assert!(
        !result
            ...
            .assumptions
            .iter()
            .any(|a| a.starts_with("do_calculus")),
...
fn provenance_hash_changes_when_delta_changes() {
    ...
    frame_a.proposed_delta_ttl =
        "@prefix ex: <http://ex.org/> . ex:A a <http://www.w3.org/2002/07/owl#Class> .".to_string();
    let mut frame_b = base_frame();
    frame_b.proposed_delta_ttl =
        "@prefix ex: <http://ex.org/> . ex:B a <http://www.w3.org/2002/07/owl#Class> .".to_string();

    let r_a = certify_action(&db, &graph, &frame_a).expect("a");
    let r_b = certify_action(&db, &graph, &frame_b).expect("b");
    assert_ne!(
        r_a.certificate.provenance_hash, r_b.certificate.provenance_hash,
    ...
    assert_eq!(
        r_a.certificate.provenance_hash,
        r_a2.certificate.provenance_hash
    );
}
