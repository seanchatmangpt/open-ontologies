...
fn test_train_simple_hierarchy() {
    ...
    assert!(
        dog_cat < dog_car,
        "Dog-Cat ({dog_cat}) should be closer than Dog-Car ({dog_car})"
    );
}
...
fn test_root_near_origin() {
    ...
    let thing_norm: f32 = embeddings["http://example.org/Thing"]
        .iter()
        .map(|x| x * x)
        .sum::<f32>()
        .sqrt();
    let dog_norm: f32 = embeddings["http://example.org/Dog"]
        .iter()
        .map(|x| x * x)
        .sum::<f32>()
        .sqrt();
