...
fn do_sql_ingest(
    ...
    if rows.is_empty() {
        return Ok(
            serde_json::json!({"ok": true, "driver": driver.as_str(), "triples_loaded": 0, "rows_processed": 0}),
        );
    }
    let cfg = if let Some(m) = mapping {
        let content = if inline_mapping {
            m.to_string()
        } else {
            std::fs::read_to_string(m).map_err(to_verb_err)?
        };
        serde_json::from_str::<MappingConfig>(&content).map_err(to_verb_err)?
    } else {
        let headers = DataIngester::extract_headers(&rows);
    ...
    let count = graph.load_ntriples(&ntriples).map_err(to_verb_err)?;
    Ok(
        serde_json::json!({"ok": true, "driver": driver.as_str(), "triples_loaded": count, "rows_processed": rows.len(), "mapping_fields": cfg.mappings.len()}),
    )
}
...
fn do_extend(
    ...
    let reason_result = profile.and_then(|p| {
        Reasoner::run(&graph, p, true)
            .ok()
            .and_then(|r| serde_json::from_str::<serde_json::Value>(&r).ok())
    });
    Ok(
        serde_json::json!({"ok": true, "triples_loaded": triples_loaded, "rows": rows.len(), "shacl": shacl_result, "reason": reason_result}),
    )
}
...
fn import_owl(
    ...
    data_dir: Option<String>,
) -> NounVerbResult<ImportOwlOutput> {
    let dir = data_dir.as_deref().unwrap_or(DEFAULT_DATA_DIR);
...
fn do_import_schema_inner(
    ...
    let count = graph
        .load_turtle(&turtle, Some(base_iri))
        .map_err(to_verb_err)?;
    Ok(
        serde_json::json!({"ok": true, "driver": driver.as_str(), "tables": tables.len(), "triples": count, "base_iri": base_iri}),
    )
}

#[cfg_attr(
    not(any(feature = "postgres", feature = "duckdb")),
    allow(unused_variables)
)]
fn fetch_schema_tables(
    connection: &str,
    driver: &open_ontologies::sqlsource::SqlDriver,
) -> NounVerbResult<Vec<open_ontologies::schema::TableInfo>> {
    match driver {
...
fn sql_ingest(
    ...
    do_sql_ingest(
        ...
        &sql_str,
        mapping.as_deref(),
        inline_mapping.unwrap_or(false),
        &base,
        dir,
    )
}
...
fn batch(
    ...
    let exit_code =
        tokio::runtime::Handle::current().block_on(runner.run(&batch_input, bail.unwrap_or(false)));
