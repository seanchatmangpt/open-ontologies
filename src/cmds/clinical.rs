...
fn load_crosswalks() -> NounVerbResult<open_ontologies::clinical::ClinicalCrosswalks> {
    open_ontologies::clinical::ClinicalCrosswalks::load(CROSSWALKS_PATH).map_err(|e| {
        clap_noun_verb::NounVerbError::execution_error(format!("Crosswalks not loaded: {}", e))
    })
}
...
fn crosswalk(code: String, system: String) -> NounVerbResult<CrosswalkOutput> {
    ...
    Ok(CrosswalkOutput {
        mappings: results
            .iter()
            .map(|r| CrosswalkMapping {
                target_code: r.target_code.clone(),
                target_system: r.target_system.clone(),
