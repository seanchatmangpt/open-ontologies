...
) -> NounVerbResult<LockOutput> {
    let iris: Vec<String> = iris_csv
        .split(',')
...
fn enforce(pack: Option<String>, data_dir: Option<String>) -> NounVerbResult<serde_json::Value> {
    ...
    let result = enforcer
        .enforce_with_feedback(&p, Some(&db))
        .unwrap_or_else(|e| format!(r#"{{"error":"{}"}}"#, e));
    serde_json::from_str::<serde_json::Value>(&result).map_err(to_verb_err)
}
...
fn monitor_clear(data_dir: Option<String>) -> NounVerbResult<MonitorClearOutput> {
    ...
    let m = open_ontologies::monitor::Monitor::new(db, graph);
    m.clear_blocked();
    Ok(MonitorClearOutput {
        ok: true,
        message: "Monitor block cleared".to_string(),
    })
}
...
fn open_db_handle(
    db_path: Option<&str>,
) -> NounVerbResult<Option<open_ontologies::state::StateDb>> {
    match db_path {
        Some(p) => Ok(Some(
...
fn maybe_attach_chain(
    ...
    let Some(rh) = hex_to_32(receipt_hash) else {
        return;
    };
