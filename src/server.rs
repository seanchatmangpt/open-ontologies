...
impl OpenOntologiesServer {
    ...
    #[tool(
        name = "onto_sql_sync_state",
        description = "Read the recorded CDC watermark for a sync_key. Returns {sync_key, last_watermark, watermark_column, last_synced_at, rows_synced, total_rows_lifetime} or null when no sync has been recorded yet. Pair with `onto_sql_ingest` — caller passes the watermark in their own WHERE clause; server tracks state."
    )]
    async fn onto_sql_sync_state(
        &self,
        Parameters(input): Parameters<OntoSqlSyncStateInput>,
    ) -> String {
    ...
    #[tool(
        name = "onto_sql_sync_reset",
        description = "Clear the recorded CDC watermark for a sync_key. Returns {removed: true} if a state row was deleted, {removed: false} if no state existed. Use when resyncing from scratch."
    )]
    async fn onto_sql_sync_reset(
        &self,
        Parameters(input): Parameters<OntoSqlSyncResetInput>,
    ) -> String {
    ...
    #[tool(
        name = "onto_sql_sync_states_list",
        description = "List every recorded CDC sync state across all sync_keys. Diagnostic helper."
    )]
